...
class UpdateStockUseCaseTest {
    ...
    @Test
    @DisplayName("Stocks 을 업데이트 시킨다.")
    void updateStocksAndRanksSuccessfully() {

        // given
        Stock existStock = stockRepository.findByCode("STOCK1").block();
        assertThat(existStock).isNotNull();
        ...
        List<Stock> givenStocks = List.of(updateStock);

        // when
        updateStockUseCase.execute(givenStocks).block();
        ...
        StepVerifier.create(stockRepository.findAll())
                .assertNext(stock -> {
                    ...
                    assertThat(stock.getViews()).isEqualTo(700L);
                })
                .assertNext(stock -> {
